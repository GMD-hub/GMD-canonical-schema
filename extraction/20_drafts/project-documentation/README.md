...
| `wiki/` | Detailed project documentation after this draft is approved and promoted. |
...
AI agents write only to `extraction/20_drafts/`. Humans own review, approval,
and promotion into `knowledge/` and `country-parameters/`. Never invent a rule
or parameter value; use `null` where the source is insufficient and document
the missing evidence in provenance.
...
The proposed wiki provides the detailed operating guide:
